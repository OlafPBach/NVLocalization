...
%Place our Main NV into the graph
scatter3(NVpos(1,1),NVpos(2,1),NVpos(3,1),40,[1,0.7,0],'filled','MarkerEdgeColor','black');
quiver3(NVpos(1,1),NVpos(2,1),NVpos(3,1),NVori(1,1),NVori(2,1),NVori(3,1),1e-8,'color',[1,0.7,0]);

%Then place the particle that got changed
switch Changes(1,2) %Look at which type of charge was changed
    case 1
        scatter3(NVpos(1,Changes(2,2)),NVpos(2,Changes(2,2)),NVpos(3,Changes(2,2)),20,'g','filled','MarkerEdgeColor','black')
        %Mark the specific charge that was changed
    case 2
        scatter3(P1pos(1,Changes(2,2)),P1pos(2,Changes(2,2)),P1pos(3,Changes(2,2)),20,'b','filled','MarkerEdgeColor','black')
    case 3
        scatter3(VCpos(1,Changes(2,2)),VCpos(2,Changes(2,2)),VCpos(3,Changes(2,2)),20,'r','filled','MarkerEdgeColor','black')
end

%Initialize out E and S values
En = Elong(2,1);    %E∥
Ep = Etran(2,1);    %E⊥
Sn = Elong(1,1);    %S∥
Sp = Etran(1,1);    %S⊥

%Solve for R using our equation before
...
R(:,tt) = [En - Sn ; Ep*cos(T) - Sp ; Ep*sin(T)] * sqrt(EC) * (En^2 + Ep^2 + Sn^2 + Sp^2 - 2*En*Sn - 2*Ep*Sp*cos(T))^(-3/4);
%±( n(E∥ - S∥) + s(E⊥cosθ - S⊥) + a(E⊥sinθ) ) * sqrt(Ec) * (E∥² + E⊥² + S∥² + S⊥² - 2E∥S∥ - 2E⊥S⊥cosθ )^(-3/4)
end

%This is in the n,s,a basis
n = NVori(:,1);
fs = cross(n,[1;0;0]);
fs = fs / norm(fs);     %s' == fraud s == fs
fa = cross(n,fs);

%We can see what our R values will look like in our "fraud space"

R1(:,:) = R(1,:).*n + R(2,:).*fs - R(3,:).*fa;
%R1 is our NV1's R value, with θ dependance & φ=0
plot3(R1(1,:),R1(2,:),R1(3,:), 'color', "k", 'LineWidth',2)

%However since we dont know the direction of either the true s or a vector,
%we have to rotate this along the NV orientation

%Impliment some phi dependancy
PhiRes = 45;
Phi = linspace(0,2*pi,PhiRes);
R1 = zeros(3,ThetaRes,PhiRes);

for pp = 1:PhiRes
P = Phi(pp);
s = fs*cos(P) + fa*sin(P);
a = fs*sin(P) - fa*cos(P);
R1(:,:,pp) = R(1,:).*n + R(2,:).*s - R(3,:).*a;
%R2 is our NV1's final R value, with both θ & φ dependance
end
...
for pp = 2:PhiRes
plot3(R1(1,:,pp),R1(2,:,pp),R1(3,:,pp), 'color', colors(pp,:))
end

%However in some instances particles charge will be of opposite sign, as we
%saw in the equations
% vec_R = ±vec_E * sqrt(Ec) * mag_E^(-3/4)
%So we also have to plot -R2 as well

for pp = 2:PhiRes
plot3(-R1(1,:,pp),-R1(2,:,pp),-R1(3,:,pp), 'color', colors(pp,:))
end

%So long as the charge is close enough this should always locate our charge
%in space


%% Step 8: Multi NV Localization
%To locate a charge in space, we have to do what we just did but with
%multiple NV centers. The process is the exact same for the most part.

%Again, start with the same graph as before, but with our first 4 NV's
figure(6)
xlabel('X (m)'); ylabel('Y (m)'); zlabel('Z (m)')
axis square
hold on
colors = [1,0.7,0 ; 1,0,0 ; 0,1,0 ; 0,0,1 ];
scatter3(NVpos(1,1:4),NVpos(2,1:4),NVpos(3,1:4),40,colors,'filled','MarkerEdgeColor','black');
quiver3(NVpos(1,1:4),NVpos(2,1:4),NVpos(3,1:4),NVori(1,1:4),NVori(2,1:4),NVori(3,1:4),'k');


%Make our resolution lower since we have more data to work with
%ThetaRes = 90;
%Theta = linspace(0,2*pi,ThetaRes);
%PhiRes = 30;
%Phi = linspace(0,2*pi,PhiRes);

%Now consider our 4 NV's
Rv = zeros(3,ThetaRes,PhiRes,4);
for vv = 1:4
%initialize our E & S values
En = Elong(2,vv);    %E∥
Ep = Etran(2,vv);    %E⊥
Sn = Elong(1,vv);    %S∥
Sp = Etran(1,vv);    %S⊥
...
s = fs*cos(P) + fa*sin(P);
a = fs*sin(P) - fa*cos(P);
Rv(:,:,pp,vv) = R(1,:).*n + R(2,:).*s - R(3,:).*a;
%Rv is our NVv's final R value, with both θ & φ dependance
end

%Now plot all our localizations
for pp = 1:PhiRes
plot3(Rv(1,:,pp,vv)+NVpos(1,vv),Rv(2,:,pp,vv)+NVpos(2,vv),Rv(3,:,pp,vv)+NVpos(3,vv), 'color', colors(vv,:))
plot3(-Rv(1,:,pp,vv)+NVpos(1,vv),-Rv(2,:,pp,vv)+NVpos(2,vv),-Rv(3,:,pp,vv)+NVpos(3,vv), 'color', colors(vv,:))
end

end

%Now plot the charge at the end so it's easier to see in the graph
%the type of charge doesnt matter anymore, so they will all be black
switch Changes(1,2) %Look at which type of charge was changed
    case 1; scatter3(NVpos(1,Changes(2,2)),NVpos(2,Changes(2,2)),NVpos(3,Changes(2,2)),20,'k','filled','MarkerEdgeColor','black')
    case 2; scatter3(P1pos(1,Changes(2,2)),P1pos(2,Changes(2,2)),P1pos(3,Changes(2,2)),20,'k','filled','MarkerEdgeColor','black')
    case 3; scatter3(VCpos(1,Changes(2,2)),VCpos(2,Changes(2,2)),VCpos(3,Changes(2,2)),20,'k','filled','MarkerEdgeColor','black')
end
